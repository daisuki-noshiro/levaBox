...
Wails bridge 已完成。前端 `ImportView` 已接入元数据确认流程：
...
→ 展示并编辑元数据
→ 展示 Tag / Cover / Background 候选
→ 选择 Cover / Background
```

## 下一阶段

根据用户最终确认的数据构造 `SaveImportRequest`，调用 `SaveImport` 完成真实前端导入。

## 暂未完成

- 前端最终 Tag 选择、`SaveImportRequest` 构造与保存流程；
- 编辑已有游戏的完整业务；
- 游戏启动 service；
- 删除游戏时的本地媒体清理；
- metadata 在线刷新；
